...
class WSINDy:
  ...
  # Soft-rank heuristic gamma^2 (effective number of independent weak-form equations)
  def compute_soft_rank(self):
    K = self.library.shape[0]
    [m, p] = [self.m[0], self.p[0]]

    # Normalized (scale-free) Bernstein test function and its derivative on n in [-1,1]
    n = torch.linspace(-1, 1, 2*m+1, dtype=torch.float64)
    phi = (1 - n**2)**p
    phi_dot = -2*p*n*(1 - n**2)**(p-1)

    # gamma := sqrt(K) * ||phi||_2^2 / ||phi_dot * phi||_2,  with gamma^2 ~ K/m
    cross = torch.from_numpy(np.convolve(phi_dot.numpy(), phi.numpy()))
    gamma = np.sqrt(K) * (phi**2).sum() / la.norm(cross)
    return (gamma**2).item()

  # Zero-admissible MSTLS: MSTLS followed by a BIC test against the trivial (zero) model
  def MSTLS_with_BIC_testing(self, gamma_squared=None, tau_b=1e-8, trigger_BIC='poor_fit',
                             residual_cutoff=0.5, Lambda=None,
                             Lambdas=10**((4/49)*torch.arange(0,50)-4)):
    G = self.library
    b = self.lhs
    b_norm = la.norm(b).item()

    # Soft-rank heuristic, the effective number of independent weak-form equations
    if gamma_squared is None:
      gamma_squared = self.compute_soft_rank()
    self.gamma_squared = gamma_squared

    # Numerical guard against ||b|| ~ 0 (e.g. an exactly-conserved quantity)
    if b_norm <= tau_b:
      w_star = torch.zeros(G.shape[1], dtype=torch.float64)
      self.Lambda = Lambda if Lambda is not None else 0.0
      self.loss = 0.0
      self.bic_delta = float('inf')
      self.bic_triggered = True
      self.coeffs = w_star
      return w_star

    # --- MSTLS: scan thresholds for lambda* (identical to self.MSTLS) ---
    ...
    if Lambda is not None:
      ...
      index = loss_history.index(min(loss_history))
      Lambda_star = Lambdas[index].item()
    [w_mstls, loss_star] = self.MSTLS_iterate(Lambda_star, w_LS.clone())

    # --- BIC test against the zero model: Delta = B(w) - B(0) ---
    residual = la.norm(b - G @ w_mstls).item()
    rel_residual = residual / b_norm

    if trigger_BIC == 'always':
      run_bic = True
    elif trigger_BIC == 'poor_fit':
      run_bic = rel_residual >= residual_cutoff
    else:
      raise ValueError("trigger_BIC must be 'poor_fit' or 'always'.")

    bic_delta = None
    if run_bic:
      support = w_mstls != 0
      rank = torch.linalg.matrix_rank(G[:, support]).item() if support.any() else 0
      gamma = np.sqrt(gamma_squared)
      resid_sq_ratio = max(rel_residual**2, 1e-300)  # guard log(0) for perfect fits
      bic_delta = gamma_squared * np.log(resid_sq_ratio) + 2*np.log(gamma)*rank
      if bic_delta >= 0:
        w_mstls = torch.zeros_like(w_mstls)  # trivial dynamics preferred

    # --- finalize (mirror self.MSTLS) ---
    w_star = self.mu * w_mstls if self.rescale else w_mstls
    self.Lambda = Lambda_star
    self.loss = loss_star
    self.bic_delta = bic_delta
    self.bic_triggered = run_bic
    self.coeffs = w_star
    return w_star
  ...
  def print_report(self):
    ...
    if hasattr(self, 'bic_delta'):
      delta = 'n/a (not triggered)' if self.bic_delta is None else f'{self.bic_delta:.3f}'
      print(f'gamma^2 = {self.gamma_squared:.3f}')
      print(f'BIC triggered = {self.bic_triggered}')
      print(f'BIC Delta = {delta}')
    return
